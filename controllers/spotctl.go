...
func (c ControllerSpot) SpotTodos(db *sql.DB) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for rows.Next() {
			clt := models.Spot{}
			err := rows.Scan(&clt.ID, &clt.Unidade, &clt.Tipo, &clt.Preco, &clt.Livre, &clt.Bloqueado, &clt.Height, &clt.Width, &clt.Y, &clt.X, &clt.Transform)
			if err != nil {
				http.Error(w, http.StatusText(500), 500)
				return
			}
...
func (c ControllerSpot) SpotUnico(db *sql.DB) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = row.Scan(&spot.ID, &spot.Unidade, &spot.Tipo, &spot.Preco, &spot.Livre, &spot.Bloqueado, &spot.Height, &spot.Width, &spot.Y, &spot.X, &spot.Transform)
		if err != nil {
			if err == sql.ErrNoRows {
				error.Message = "Spot inexistente"
				utils.RespondWithError(w, http.StatusBadRequest, error)
